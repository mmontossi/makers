...
The idea behind the generators it's to separate them from fabricators en use them as attributes, so you would be force to not use them inside your fabricators definition to ensure readability:
  Fabricators.define do
...
Both one to many and one to many association work more naturally than factory_girl:
  Fabricators.define do
    fabricator :user do
      posts # Be default 1 record
      comments 4 # You can override this if you like
    end
    fabricator :post do
      user
    end
